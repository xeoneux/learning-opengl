...
const char *frag =
    GLSL(120, varying vec4 dstColor; void main() { gl_FragColor = dstColor; });
...
struct MyApp : public App {
  ...
  void init() {
    ...
    Vertex triangle[] = {{glm::vec2(-1, 0), glm::vec4(1, 0, 0, 1)},
  ...
  void onDraw() {
    ...
    glm::mat4 proj =
        glm::perspective(PI / 3.0f, window().ratio(), 0.1f, -10.0f);
    ...
    for (int i = 0; i < 100; ++i) {
       ...
       *-----------------------------------------------------------------------------*/
      glm::mat4 translate =
          glm::translate(glm::mat4(), glm::vec3(sin(time), 0, (float)i / 100));
      glm::mat4 rotate =
          glm::rotate(glm::mat4(), time * PI * i / 100, glm::vec3(0, 0, 1));
      glm::mat4 scale =
          glm::scale(glm::mat4(), glm::vec3(1.0f - (float)i / 100));
